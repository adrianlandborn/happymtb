...
//				showDialog(DIALOG_FETCH_KOS_ERROR);
	...
	private void FillList() throws IOException {
		...
		if (mKoSObjectItem.getType().contains("S�ljes")){
			int identifier = getResources().getIdentifier("rowshapegreen", "drawable","org.pebeijer.happymtb");
			mBackgroundColor.setBackgroundResource(identifier);
		} else {
			int identifier = getResources().getIdentifier("rowshapered", "drawable","org.pebeijer.happymtb");
			mBackgroundColor.setBackgroundResource(identifier);
		}		
	}	
