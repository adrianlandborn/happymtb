...
import pebeijer.happymtb.view.KoSRowView;
...
public class ListKoSAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mKoSItems.get(position).getTitle().charAt(0) == 'S'){
			int identifier = mContext.getResources().getIdentifier("rowshapegreen", "drawable","pebeijer.happymtb");
			KSRowV.setRowBackgroundColor(identifier);
		} else {
			int identifier = mContext.getResources().getIdentifier("rowshapered", "drawable","pebeijer.happymtb");
			KSRowV.setRowBackgroundColor(identifier);
		}		
