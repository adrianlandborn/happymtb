...
import pebeijer.happymtb.task.KoSObjectImageTask;
import pebeijer.happymtb.task.KoSObjectTask;
import pebeijer.happymtb.KoSObjectActivity;
...
public class KoSObjectFragment extends Fragment implements DialogInterface.OnCancelListener {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		menu.clear();		
		inflater.inflate(R.menu.kosobjectmenu, menu);
...
//				showDialog(DIALOG_FETCH_KOS_ERROR);
	...
	private void FillList() throws IOException {
		...
		if (mKoSObjectItem.getType().contains("Säljes")){
			int identifier = getResources().getIdentifier("rowshapegreen", "drawable","pebeijer.happymtb");
			mBackgroundColor.setBackgroundResource(identifier);
		} else {
			int identifier = getResources().getIdentifier("rowshapered", "drawable","pebeijer.happymtb");
			mBackgroundColor.setBackgroundResource(identifier);
		}		
	}	
